...
def qpos_for_frame(points: np.ndarray, rest: np.ndarray, rest_basis: np.ndarray, human) -> np.ndarray:
    """Retarget one keypoint frame to the MJCF body's local transforms.

    The MJCF generated by ``generate_human_mjcf.py`` stores each bone as a
    child body's fixed ``pos`` in its parent body frame.  Consequently, a
    body's ball joint controls that body's orientation and the positions of
    its descendants; it does not rotate the body's own anchor around the
    parent anchor.  The previous implementation assigned the rotation of
    ``parent -> child`` to the child's joint, which is one level too late and
    progressively displaced the knees, ankles, and feet.

    The root pose is the target pelvis frame itself.  For every non-terminal
    body, its local rotation is chosen from its first outgoing bone.  This is
    sufficient for the single-child leg chains and provides a deterministic
    twist choice for branching torso chains; keypoints alone cannot determine
    twist around a bone axis.
    """
    root_target_basis = rotation_from_basis(points)
    global_rotations: list[np.ndarray | None] = [None] * len(BODY_NAMES)
    global_rotations[0] = root_target_basis

    children: list[list[int]] = [[] for _ in BODY_NAMES]
    for child in range(1, len(BODY_NAMES)):
        children[int(PARENTS[child])].append(child)

    qpos = np.zeros(human.n_qs, dtype=np.float32)
    free = human.get_joint(name="pelvis_free")
    qpos[free.q_start : free.q_start + 7] = np.concatenate(
        [points[0], quat_wxyz(root_target_basis)]
    )
    ...
    for child in range(1, len(BODY_NAMES)):
        ...
        if parent_rotation is None:
            raise RuntimeError(f"Missing parent rotation for {BODY_NAMES[child]}")

        outgoing = children[child]
        if outgoing:
            next_child = outgoing[0]
            rest_vector = rest_basis.T @ (rest[next_child] - rest[child])
            target_vector = parent_rotation.T @ (points[next_child] - points[child])
            local_q = quat_from_two_vectors(rest_vector, target_vector)
        else:
            # A terminal keypoint has no outgoing bone to constrain its twist.
            # Keep its local orientation neutral and let its parent determine
            # the terminal anchor position.
            local_q = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32)

        local_rotation = Rotation.from_quat([local_q[1], local_q[2], local_q[3], local_q[0]]).as_matrix()
        global_rotations[child] = parent_rotation @ local_rotation
...
def run(
    ...
    for step in range(steps + 1):
        ...
        # Warm-start and limit only the IK-controlled legs.  In the plain
        # retargeting mode the corrected qpos is already the direct solution
        # for this frame; blending it with the previous leg pose would add a
        # visible lag (especially on the swing foot).
        if leg_ik and previous_applied_leg_qpos is not None:
